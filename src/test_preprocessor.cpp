...
TEST(zscore_basic) {
    ...
    for (size_t i = 0; i < 8; i++) vol.data[i] = 0.0f;
    vol.data[8]  = 10.0f;
    vol.data[9]  = 20.0f;
    vol.data[10] = 30.0f;
    vol.data[11] = 40.0f;
    ...
    double sq_sum = 0;
    for (size_t i = 8; i < 16; i++) sq_sum += vol.data[i] * vol.data[i];
...
TEST(zscore_all_zero) {
    ...
    for (size_t i = 0; i < vol.data.size(); i++) assert(vol.data[i] == 0.0f);
}

TEST(single_patch_small_volume) {
    size_t C = 2, D = 4, H = 4, W = 4;
    std::vector<float> data(C * D * H * W);
    std::iota(data.begin(), data.end(), 0.0f);

    Preprocessor p(8, 0.5f);
    auto grid = p.extract_patches(data, static_cast<int>(C),
                                  static_cast<int>(D), static_cast<int>(H), static_cast<int>(W));

    assert(grid.patches.size() == 1);
    assert(grid.patch_d == 8);
    assert(grid.padded_d >= static_cast<int>(D));
    assert(grid.padded_h >= static_cast<int>(H));
    assert(grid.padded_w >= static_cast<int>(W));
...
TEST(overlap_produces_multiple_patches) {
    ...
    for (auto& patch : grid.patches) {
        if (patch.origin_x < static_cast<int>(W) &&
            patch.origin_y < static_cast<int>(H) &&
            patch.origin_z < static_cast<int>(D)) {
